...
gasto_sin_overhead_grouped = data0[data0['Proceso'] != 'Overhead'].groupby(['Ejercicio', 'Período', 'Proceso'])['Valor/mon.inf.'].sum().reset_index()
gasto_sin_overhead_totales = data0[data0['Proceso'] != 'Overhead'].groupby(['Ejercicio', 'Período'])['Valor/mon.inf.'].sum().reset_index()

# Asegurar que los nombres de las columnas coinciden para la división
gasto_sin_overhead_totales = gasto_sin_overhead_totales.rename(columns={'Valor/mon.inf.': 'Total'})

# Realizar la división para obtener las proporciones
proporciones = gasto_sin_overhead_grouped.merge(gasto_sin_overhead_totales, on=['Ejercicio', 'Período'])
proporciones['Proporción'] = proporciones['Valor/mon.inf.'] / proporciones['Total']

# Paso 4: Distribuir el gasto "Overhead" proporcionalmente por año y período
gasto_real_overhead = gasto_real_overhead.merge(proporciones[['Ejercicio', 'Período', 'Proceso', 'Proporción']], on=['Ejercicio', 'Período'], how='left')
